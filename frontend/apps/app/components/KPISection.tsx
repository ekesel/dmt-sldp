...
export const KPICard = ({ label, value, trend, description }: KPIProps) => {
    const getSentimentColor = () => {
        ...
        if (trend.sentiment) {
            return trend.sentiment === 'positive' ? 'text-emerald-500' :
                trend.sentiment === 'negative' ? 'text-destructive' : 'text-muted-foreground';
        }
        return trend.direction === 'up' ? 'text-emerald-500' :
    ...
    return (
        <Card className="flex flex-col gap-3 p-5 hover:border-primary/40 transition-colors duration-200 group bg-card border-border">
            <h3 className="text-muted-foreground text-sm font-medium group-hover:text-foreground/80 transition-colors uppercase tracking-wider">{label}</h3>
            ...
            {description && <div className="text-muted-foreground text-xs mt-auto font-medium leading-relaxed border-t border-border pt-2">{description}</div>}
        </Card>
    );
};
