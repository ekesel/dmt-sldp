...
export const SyncProgressOverlay: React.FC<SyncProgressOverlayProps> = ({ isOpen, onClose, projectId, tenantId }) => {
    ...
    return (
        <div className={`fixed inset-0 z-[100] flex items-center justify-center bg-background/80 backdrop-blur-md transition-opacity duration-300 ${isOpen ? 'opacity-100' : 'opacity-0'}`}>
            <div className="bg-popover border border-border rounded-2xl shadow-2xl p-8 max-w-md w-full mx-4 transform transition-all duration-300 scale-100">
                <div className="flex justify-between items-start mb-6">
                    <div>
                        <h3 className="text-xl font-bold text-foreground mb-1">Project Resync</h3>
                        <p className="text-sm text-muted-foreground">Synchronizing data from active sources</p>
                    </div>
                </div>
                ...
                <div className="relative pt-1">
                    <div className="flex mb-2 items-center justify-between">
                        <div>
                            <span className={`text-xs font-semibold inline-block py-1 px-2 uppercase rounded-full ${syncState.status === 'failed' ? 'text-destructive bg-destructive/10' :
                                syncState.status === 'success' ? 'text-accent bg-accent/10' :
                    ...
                    <div className="overflow-hidden h-2 mb-4 text-xs flex rounded bg-muted/20">
                        <div
                            ...
                            className={`shadow-none flex flex-col text-center whitespace-nowrap text-primary-foreground justify-center transition-all duration-500 ${syncState.status === 'failed' ? 'bg-destructive' :
                                syncState.status === 'success' ? 'bg-accent' :
                                    'bg-primary'
                                }`}
                        ></div>
                    </div>
                ...
                {/* Status Message */}
                <div className={`flex items-start gap-3 p-4 rounded-xl mb-8 ${syncState.status === 'failed' ? 'bg-destructive/5 text-destructive' :
                    syncState.status === 'success' ? 'bg-accent/5 text-accent' :
                        'bg-muted/10 text-muted-foreground'
                    }`}>
                    {syncState.status === 'in_progress' && <Loader2 size={18} className="animate-spin text-primary shrink-0" />}
                    {syncState.status === 'success' && <CheckCircle size={18} className="text-accent shrink-0" />}
                    {syncState.status === 'failed' && <XCircle size={18} className="text-destructive shrink-0" />}
                    {syncState.status === 'pending' && <AlertTriangle size={18} className="text-warning shrink-0" />}
                ...
                <div className="flex justify-end">
                    <button
                        onClick={onClose}
                        className="px-6 py-2 rounded-lg font-bold text-sm transition-all bg-secondary text-secondary-foreground hover:bg-secondary/80"
                    >
