...
const StaticPostCard = ({ user, time, content, image, likes, comments, shares }: {
    ...
    return (
        <div className="bg-card border border-border rounded-xl shadow-lg mb-4 overflow-hidden animate-in fade-in slide-in-from-bottom-4 duration-500">
            ...
            <div className="flex items-center justify-between p-4">
                <div className="flex items-center gap-3">
                    ...
                    <div className="flex flex-col">
                        <span className="text-foreground font-bold text-sm hover:underline cursor-pointer">{user.name}</span>
            ...
            {image && (
                <div className="relative group cursor-pointer border-t border-b border-border/50 bg-muted/50">
                    <img 
                        src={image} 
                        alt="Post media" 
                        className="w-full max-h-[500px] object-cover transition-transform duration-700 group-hover:scale-[1.01]" 
                    />
                </div>
            )}
            ...
            <div className="px-1 py-1 flex items-center justify-between">
                {[
                    { label: "Like", icon: "M14 9l-3.5 4.5-3-3.5-5.5 8.5h24l-12-16z" }, // Using placeholder icons for simplicity
                    ...
                    { label: "Share", icon: "M15 15l6-6m0 0l-6-6m6 6H9a6 6 0 000 12h3" }
                ].map((action, idx) => (
                    <button key={idx} className="flex-1 flex items-center justify-center gap-2 py-2 text-muted-foreground font-semibold hover:bg-muted rounded-lg mb-0.5 transition-all active:scale-95">
                        {idx === 0 ? (
