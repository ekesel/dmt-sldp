...
export const DeepSprintAnalysisContent: React.FC<
  DeepSprintAnalysisContentProps
> = ({ overallHealth, assigneeInsights, riskFactors }) => {
  return (
        ...
        <div className="lg:col-span-2 space-y-4">
          ...
          <div className="grid grid-cols-1 gap-4">
            {assigneeInsights.map((insight, idx) => (
              <Card
                key={idx}
                className="p-5 bg-card border-border hover:border-primary/30 transition-all group"
              >
        ...
        <div className="space-y-4">
          ...
          <div className="space-y-3">
            {riskFactors.length > 0 ? (
              riskFactors.map((risk, idx) => (
                <div
                  key={idx}
                  className="flex items-start gap-3 p-4 rounded-xl bg-destructive/5 border border-destructive/10 text-destructive"
                >
          ...
          <div className="mt-8 p-4 rounded-xl bg-muted/30 border border-border">
            ...
            <p className="text-xs text-muted-foreground leading-relaxed">
              This analysis is based on story point distribution, cycle time
              trends, and AC quality metrics recorded during this sprint period.
            </p>
          </div>
        </div>
      </div>
