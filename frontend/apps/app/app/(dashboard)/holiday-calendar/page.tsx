...
export default function HolidayCalendarPage() {
    ...
    const getFileName = (url: string) => {
        if (!url) return 'Holiday_Calendar.pdf';
    ...
    const handleCreateFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        setUploading(true);
        const toastId = toast.loading('Uploading holiday calendar...');
        try {
            const formData = new FormData();
            formData.append('holiday_calendar_file', file);
                    ...
                    <div className="flex items-start gap-4">
                        ...
                        <div className="space-y-1">
                            <div className="flex items-center gap-2">
                                {/* Custom Icon */}
                                <Palmtree className="w-7 h-7 text-primary" strokeWidth={2.5} />
                            ...
                            <p className="text-muted-foreground text-[0.875rem] font-medium leading-normal">
                                Stay updated with the official list of holidays and company closures.
                            </p>
                        </div>
                    </div>
                ...
                {loading ? (
                    ...
                    <div className="space-y-6">
                        {calendars.map((calendar) => {
                            const fileName = getFileName(calendar.holiday_calendar_file);
                            const fileUrl = getFileUrl(calendar.holiday_calendar_file);
                                ...
                                >
                                    ...
                                    <div className="flex gap-4 items-start pr-8">
                                        ...
                                        <div className="p-3.5 rounded-xl bg-primary/10 text-primary shrink-0 border border-primary/20">
                                            <Palmtree className="w-7 h-7" strokeWidth={2.2} />
                                        </div>
                                    ...
                                    {isImage ? (
                                        ...
                                        <div className="w-full overflow-hidden rounded-xl bg-muted border border-border/80 relative min-h-[400px] mt-2 shadow-[inset_0_2px_4px_rgba(0,0,0,0.02)] flex items-center justify-center">
                                            <iframe
                                                ...
                                                title="Holiday Calendar PDF Viewer"
                                            />
                                        </div>
                                    )}
                    ...
                    <div className="flex flex-col items-center justify-center text-center p-12 bg-card text-card-foreground rounded-2xl border border-border shadow-sm min-h-[22rem]">
                        ...
                        <h3 className="text-[1.125rem] font-bold text-foreground">No Holiday Calendar Uploaded</h3>
                        <p className="text-muted-foreground text-[0.875rem] max-w-sm mt-2 font-medium leading-normal">
                            {isManager
                                ? 'Get started by uploading the first official holiday activities & events calendar!'
                                : 'There are currently no holiday calendars uploaded by management.'
                            }
                        </p>
                    </div>
                )}
